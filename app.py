...
def get_tips(symptom_names):
    ...
    for name in symptom_names:
        ...
        print(f"🔎 Searching for symptom: '{name}' in the database")

        symptom = Symptom.query.filter(Symptom.name.ilike(name)).first()
        
        if symptom:
            print(f"🟢 Found: {symptom.name} - Tip: {symptom.tip}")
            if symptom.tip:
                tips.append(symptom.tip)
        else:
            print(f"🔴 No symptom found for name: '{name}' in DB")
